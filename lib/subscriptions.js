import { getUser, updateUser, setSubscription, getSubscription } from './db.js';
...
export async function updateSubscription(userId, subscriptionData) {
  ...
  if (subscriptionId) {
    await setSubscription(subscriptionId, {
      userId,
      plan: updates.subscriptionPlan || subscriptionData.plan,
      status: updates.subscriptionStatus || subscriptionData.status,
      createdAt: subscriptionData.createdAt || Date.now()
    });
  }
...
// getSubscription is now imported from db.js
